// Laço para permitir múltiplas entradas de heróis
for (let i = 0; i < 3; i++) {
    let nomeHeroi = prompt("Digite o nome do herói:");
    let experienciaHeroi = parseInt(prompt("Digite a experiência do herói:"));
    classificarHeroi(nomeHeroi, experienciaHeroi);
}



// Função para classificar o herói
function classificarHeroi(nome, experiencia) {
    let nivel;

    // Estrutura de decisão para determinar o nível do herói
    if (experiencia < 1000) {
        nivel = "Ferro";
    } else if (experiencia >= 1001 && experiencia <= 2000) {
        nivel = "Bronze";
    } else if (experiencia >= 2001 && experiencia <= 5000) {
        nivel = "Prata";
    } else if (experiencia >= 5001 && experiencia <= 7000) {
        nivel = "Ouro";
    } else if (experiencia >= 7001 && experiencia <= 8000) {
        nivel = "Platina";
    } else if (experiencia >= 8001 && experiencia <= 9000) {
        nivel = "Ascendente";
    } else if (experiencia >= 9001 && experiencia <= 10000) {
        nivel = "Imortal";
    } else {
        nivel = "Radiante";
    }


    // Exibir mensagem com o nome e nível do herói
    console.log(`O Herói de nome ${nome} está no nível de ${nivel}`);
}


// Exemplo de uso da função
let nomeHeroi = "Aragorn";
let experienciaHeroi = 7500;


// Chama a função para classificar e mostrar o nível do herói
classificarHeroi(nomeHeroi, experienciaHeroi);


